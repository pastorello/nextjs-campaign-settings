...
- **Next.js App Router.** Server Components read from Postgres via Prisma. Server Actions write. No REST layer for domain data; the route handlers are one DELETE endpoint per domain (ten today, 5e and Daggerheart together), two read-only country/GeoJSON endpoints, two for map images — an authenticated `GET` that streams an uploaded map and the upload endpoint itself ([ADR-0008](./docs/adr/0008-map-image-storage.md)) — and three for record images: the upload, a read by key and a read by id ([ADR-0017](./docs/adr/0017-record-images.md)). Counts rot: `find app/api -name route.ts` is the current list, and the only one to trust.
...
8. **Styling is Tailwind utility classes, never inline `style`.** This applies even outside JSX: `app/modules/maps/**`'s Leaflet layers build marker/popup content as raw HTML strings (`L.divIcon`'s `html`, `marker.bindPopup(...)`), not JSX — but Tailwind v4 detects sources automatically (there is no `content` glob to configure — `tailwind.config.ts` is gone as of TD-112; see `app/ui/global.css`'s `@theme`/`@plugin` for the tokens it used to hold), and its scanner matches class-name-shaped strings in any file it reads, regardless of whether they sit inside a `className` attribute or a template literal. Use `class="w-8 h-8 bg-purple-600 rounded-full ..."` there, arbitrary values (`border-[3px]`, `shadow-[0_3px_8px_rgba(0,0,0,0.3)]`) where the default scale doesn't have an exact match — never `style="width: 32px; ..."`. Found 2026-08-07 in `useLinkedEntityMarkers.ts` (TD-70) and fixed before merge; flagged by the maintainer as a rule that was true all along but never written down.
   **Exception, added 2026-09-17 (TD-132):** a value that only exists at runtime — a screen coordinate, not a design token — has no Tailwind class to move to. `PlacePopover.tsx`'s `style={{ left: screenPosition.x, top: screenPosition.y }}` positions the popover at the cursor's click point; there is no finite set of classes that could express an arbitrary cursor position. This is narrow: it covers a genuine runtime position (or size — see `Spinner.tsx`'s percentage-based rewrite for a case that turned out _not_ to need the exception), not a value that's merely inconvenient to express as a class.
...
- **2026-09-09 — `POI.id` stays a stable client key; do not "reconcile" it onto the row when the create resolves.** TD-108's own card called that the better fix and read SPEC-002 §9 as having half-built it. Both were wrong: §9 _warns_ about the swap ("where a bug would hide"), and `usePOIManager`'s note 1 records the stable key as the deliberate choice — `markersRef` is keyed by that id, so re-keying orphans the marker. TD-108 was fixed the other way, by having the hook hand the resolved row id to `onPOIClick`. Reversing this needs an ADR and an atomic re-key of `markersRef`/`operationsRef`/`serverIdsRef`, not a debt fix.
- **2026-09-10 — The map's URL is written with `replaceState`, so back leaves the map; do not switch it to `pushState` as a fix.** TD-82 weighed both and the DM chose this: "up" climbs the tree, back leaves it, and the two never have to agree after a search deep link. Back retracing each hop needs the stack rebuilt from the URL on `popstate`, including places no longer in it, and a client path to the server-only `fetchPlaceAncestryChain` — a spec, not a tweak.
- **2026-09-11 — The `revalidatePath` calls are not dead code, though nothing is cached and no path matches; do not delete them.** Any call flags the Server Action as revalidated, whatever it names, and that flag is the only thing making Next re-render the page with the action's response. TD-105 once recommended deleting them on the "inert" reading; the choice is `refresh()` or corrected paths, per its correction. **Resolved 2026-09-17:** corrected paths won, behind one helper (`revalidateDashboard` in `app/lib/utils/revalidateDashboard.ts`) rather than ~50 corrected literals — see [ADR-0014](./docs/adr/0014-corrected-revalidate-paths-behind-a-helper.md). `refresh()` stays a possible later move, contained to that one file; do not re-propose switching every call site to it without the back/forward check against a production build the ADR leaves open.
- **2026-09-11 — Other game systems go in this repo, not in a fork.** SPEC-018's first draft proposed forking the app for Daggerheart, and the DM rejected it: geography, NPCs and deities are one world played under several rule sets (5e, Daggerheart, later PF2), and a fork would duplicate that world. Do not re-propose a fork to escape the cost of a system dimension — that cost is what [SPEC-018](./docs/specs/018-game-systems.md) is about. The same spec also records that the public repo holds rules structure, never rules content (DPCGL 2.0).
...
- **2026-09-17 — TD-112's `tailwind.config.ts` colour overrides are not carried into `global.css`; do not re-add them as a "completeness" fix.** They landed, then were reverted in the same PR: `e2e/a11y.spec.ts` failed `color-contrast` on every dashboard page once `blue-600` actually resolved to the config's `#2F6FEB` instead of v4's default — the active sidebar link's text-on-`sky-100` dropped from 4.58:1 to 3.98:1, under the 4.5:1 AA floor. Because the config was never loaded before TD-112, nobody had ever seen these colours rendered, so restoring them was a new visual change, not a restoration; re-tuning the dozen other `blue-600` call sites individually (pagination, several links, the map panels) was judged riskier with no dev server to check against. `docs/TECH_DEBT.md`'s TD-112 entry has the numbers.
- **2026-09-17 — Not every hand-rolled `<button>` moved onto `BaseButton` in TD-117; the geography/map ones and two text-style triggers were left alone on purpose.** `app/ui/geography/*`'s buttons are the item's own "map-control icon buttons" carve-out; `sidenav.tsx`'s sign-out button matches `NavLinks`' tile look exactly, which no `BaseButton` variant reproduces; `AssignLocationButton.tsx`'s `variant="text"` trigger is deliberately plain underlined text (SPEC-007 T3), not a button in any variant. Don't re-propose folding these in without a design reason beyond "for consistency" — TD-117's own fix text says to leave a hand-rolled button alone when no variant fits, and none of these three do.
- **2026-09-18 — The vendored map utilities' Earth-geometry maths stays, kept per the DM, 2026-09-18.** `coordinates.ts`/`maps.ts`/`validation.ts`'s unused haversine/±90/±180 helpers (TD-131) are not deleted, only marked vendored and off-limits for this app's pixel maps.
- **2026-09-18 — Deleting a landmark from the map popover now confirms first, reversing SPEC-016 T7's original call.** §5's "deleting and re-creating a landmark is cheap" was the reasoning for shipping it unconfirmed; the DM changed the answer (TD-140). Don't cite that old reasoning to remove the new confirmation dialog.
- **2026-09-17 — No `(parent, position)` unique index for the reorder actions (TD-125).** The duplicate-id bug is closed at the application level in `validateAndReorder.ts` (a `Set`-size check), not with a DB constraint: a unique index would need a migration and would fight the in-transaction position swap, where two rows briefly need to be reachable under positions that will not stay theirs until every update in the transaction has run.
- **2026-09-19 — A domain card's `level`/`type` are `cardLevel`/`cardType` in code, `@map`ped to the spec's columns (SPEC-021 T3).** `pageMetaFields` is one flat namespace, so a second `level` or `type` would silently overwrite spells' or magic items'; `DomainMetaPairs` now checks the Daggerheart metas too. Name new catalogue fields so they do not collide, rather than namespacing the registry.
- **2026-09-22 — This app builds settings; it does not run sessions.** The DM dropped the initiative tracker, the dice roller and the random generators, and had `docs/specs/001-combat-tracker.md` and `docs/domain/5e-combat.md` deleted (both stubs). The encounter builder stays, by explicit exception: it is prep, not play. The test for a new feature is whether it is used while designing the world or while playing in it — `docs/ROADMAP.md`'s _Explicitly not planned_ has the reasoning.
- **2026-09-19 — SPEC-019 T6 closed with no shadow column and no `regexp_replace`: search reads only names and titles.** No free-text search touches a formatted (`RichText`) column, so markup cannot match; `searchIgnoresMarkup.test.ts` pins it. Stripping tags from columns nobody searches is not a fix — if description search is ever specified, it must match `richTextToPlainText` output, and that test is where it fails first.
- **2026-09-24 — "Elimina definitivamente" is not a cascade; SPEC-023 shipped option A.** The DM weighed deleting the whole branch and chose to keep SPEC-010 rule 2 — children move up to the grandparent and lose their position — because that leaves a recovery path (the unpositioned pool) where a recursive delete leaves none. Don't re-propose the cascade as a fix for "it did not delete what I pointed at"; it would be a new spec.
- **2026-09-24 — a place's two popover entries are deliberately one.** "Sposta nei luoghi non posizionati" and "Rimuovi definitivamente" are now the two named outcomes of one "Rimuovi" dialog (SPEC-023), for a landmark too; adding a second destructive entry back "for speed" undoes the spec. `DeletePlaceButton` is `RemovePlaceDialog` and `geography.deletePlace` is `geography.removePlace` since the same change.
...
```
app/api/**                    route handlers (DELETE, GeoJSON, map and record images)
