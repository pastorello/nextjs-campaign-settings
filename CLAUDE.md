...
- **2026-07-22 — Unused is not dead: ask before deleting.** Some unused code is scaffolding for features not yet built. `app/modules/maps/components/map/` is a vendored library (taken from a GitHub project); `WorldMap.tsx` is a thin MVP that will wire more of it up over time, so its unused imports and unwired handlers are intentional. `BaseButton.buttonState` is a planned feature (loading / active / disabled / flat states). Do not remove an unused component, prop or import on the strength of "nothing references it" — distinguish _dead_ (tutorial leftover, superseded duplicate) from _unwired_ (waiting to be used), and when unsure, ask.
