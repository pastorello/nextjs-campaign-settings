...
vi.mock("@/app/lib/connections/prisma", () => ({
  default: {
    npc: { update, updateMany, findUnique: entityFindUnique },
    poi: { findUnique: poiFindUnique },
    zone: { findUnique: zoneFindUnique },
  },
}));

import assignNpcLocation from "./assignLocation";
...
describe("assignNpcLocation", () => {
  ...
  it("rejects an unauthenticated request without writing", async () => {
    ...
    await expect(
      assignNpcLocation({ id: 1, zoneId: null, poiId: null })
    ).rejects.toBeInstanceOf(UnauthorizedError);
    expect(update).not.toHaveBeenCalled();
  });

  it("rejects invalid input without writing", async () => {
    const result = await assignNpcLocation({
      id: 1,
      zoneId: null,
      poiId: 9,
    });
  ...
  it("assigns a zone with no poi", async () => {
    zoneFindUnique.mockResolvedValue({ id: 5 });

    const result = await assignNpcLocation({ id: 1, zoneId: 5, poiId: null });
  ...
  it("wraps a write failure in a DatabaseError", async () => {
    ...
    await expect(
      assignNpcLocation({ id: 1, zoneId: 5, poiId: null })
  ...
  it("says a missing row is missing rather than already placed", async () => {
    ...
    const result = await assignNpcLocation({ id: 404, zoneId: 5, poiId: null });
