...
for (const key of entityFieldKeys(PageType.Npc)) {
  validRow[key] = fieldMeta[key]?.defaultValue;
}

describe("fetchFilteredNpc (TD-38)", () => {
  it("returns rows for a well-formed result set", async () => {
    findMany.mockResolvedValue([validRow]);

    const { fetchFilteredNpc } = await import("./fetchFilteredNpc");
    const result = await fetchFilteredNpc(Promise.resolve({}));
  ...
  it("throws a DatabaseError instead of returning a malformed row", async () => {
    ...
    await expect(fetchFilteredNpc(Promise.resolve({}))).rejects.toThrow(
      DatabaseError
    );
  });
