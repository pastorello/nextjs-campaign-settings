...
describe("renderFieldValue", () => {
  it("renders an integer field through its options list", () => {
    expect(renderFieldValue(SpellMetaField.level, 3)).toBe("3° Livello");
  });

  it("renders an array field as a joined list of labels", () => {
    // classi 1 and 4 are Chierico and Paladino — the pairing the seeded
    // "Aiuto" carries.
    expect(renderFieldValue(SpellMetaField.classes, [1, 4])).toBe(
      "Chierico, Paladino"
    );
  });

  it("renders a boolean field as Italian yes/no", () => {
    expect(renderFieldValue(MagicItemMetaField.attuned, true)).toBe("Sì");
    expect(renderFieldValue(MagicItemMetaField.attuned, false)).toBe("No");
  });
  ...
  it("renders a string field that has options through its label", () => {
    ...
    expect(renderFieldValue(SpellMetaField.range, "30")).toBe("9 Metri");
  });

  it("renders a string field without options unchanged", () => {
    expect(renderFieldValue(SpellMetaField.name, "Dardo Incantato")).toBe(
      "Dardo Incantato"
    );
  });
  ...
  it("returns an empty label for a value outside the options list", () => {
    ...
    expect(renderFieldValue(DeityMetaField.residence, 999)).toBe("");
  });
});
