...
- **Catalogues.** A `pagesConfig` entry with a `system` belongs to that system; one without is shared. The catalogue route directories (`spells`, `magicitems`, `treasures` and their `admin/` twins) each have a one-line `layout.tsx` calling `assertPageSystem`, which 404s the page under any other system. `isPageInSystem` (`app/lib/config/`) is the one predicate behind that 404, search, the switch, the sidebar entries (`nav-links.tsx`'s `page`) and the overview cards, so none of them offers a page the system does not have. Daggerheart's tables carry a `dh` prefix (`dhDomain`, …; SPEC-021 §6) so unprefixed tables keep meaning 5e.
