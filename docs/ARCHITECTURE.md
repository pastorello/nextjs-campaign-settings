...
Every dashboard URL names a locale and a game system: `/dashboard/dnd5e/spells` (Italian, unprefixed under next-intl's `localePrefix: "as-needed"`) and `/en/dashboard/dnd5e/spells`. The pages live in `app/[locale]/dashboard/[system]/**`; `[system]/layout.tsx` 404s a param that is not in `GAME_SYSTEMS` (`app/lib/definitions/GameSystem.ts`: `dnd5e` and, since SPEC-021 T1, `daggerheart`) and renders `SideNav`, whose `SystemSwitcher` changes the segment (`i18n/switchSystemPath.ts`).
...
- **Catalogues.** A `pagesConfig` entry with a `system` belongs to that system; one without is shared. The catalogue route directories (`spells`, `magicitems`, `treasures`, Daggerheart's `domains` and `domain-cards`, and their `admin/` twins) each have a one-line `layout.tsx` calling `assertPageSystem`, which 404s the page under any other system. `isPageInSystem` (`app/lib/config/`) is the one predicate behind that 404, search, the switch, the sidebar entries (`nav-links.tsx`'s `page`) and the overview cards, so none of them offers a page the system does not have. Daggerheart's tables carry a `dh` prefix (`dhDomain`, …; SPEC-021 §6) so unprefixed tables keep meaning 5e.
...
Each `create*` / `update*` mutation `safeParse`s first and returns a `MutationResult` — `{ ok: true }`, or `{ ok: false, errors }` carrying a field-keyed map of **message keys, never prose** (`FieldErrors`, TD-124): `toFieldErrors(parsed.error)` turns Zod's issues into keys under `common.fieldErrors`, a schema's custom `message` is itself such a key, and the data layer's own refusals are written with `fieldError("key", values)`. The keys are listed once in `FIELD_ERROR_KEYS`; `FormErrorSummary` / `BespokeFormErrorSummary` (or `resolveFirstFieldError`, for a one-line toast) translate them at the render boundary, per ADR-0007. Nothing reaches Prisma on failure. Route `:id` segments go through `parseIdParam`, which returns 400 rather than letting `parseInt("abc")` reach a query as `NaN`.
