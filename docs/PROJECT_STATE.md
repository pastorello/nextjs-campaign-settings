...
- **✅ The local dev database's migration history is intact.** It was broken for a long stretch — two migrations unapplied, `migrate dev`/`migrate deploy` both failing against it, every schema change hand-applied — and this entry described that as ongoing and **TD-63** as open. TD-63 closed it on 2026-08-08. Verified against the running database on 2026-08-13: `prisma migrate status` reports all fifteen migrations applied and "Database schema is up to date!". _(The stale line survived the 2026-08-13 documentation audit that fixed its neighbours, which is the same lesson again — a status reads as settled prose and nobody re-checks it. Re-check it with the command, not the register.)_
