...
- **The calendar and the timeline.** The DM plans a campaign partly as a dated sequence of events — what the antagonists do while the party does something else — and wants that as a feature rather than as prose in a field. To be specified as SPEC-014; [SPEC-013](./specs/013-campaign-management.md) deliberately ships only a free-text `timeline` field on an adventure and marks it provisional, so the two do not collide. A **session diary** (what actually happened, written after play) is a third, distinct feature, not yet specified.
