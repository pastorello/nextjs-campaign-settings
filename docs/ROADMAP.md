...
- **Rich text in descriptions.** Shipped 2026-09-19 as [SPEC-019](./specs/019-formatted-text.md): the sanitiser and stored format ([ADR-0016](./adr/0016-formatted-text-as-sanitised-html.md)), a formatted renderer that supersedes TD-76's interim fix, the toolbar editor (`ControlType.RichText`) and the record-link picker, applied to every description field. See the spec's §11 for the full outcome.
- **Image uploads.** Shipped 2026-09-19 as [SPEC-020](./specs/020-record-images.md): one image per NPC, deity, magic item, treasure, faction and place — stripped and resized server-side, stored behind authenticated routes ([ADR-0017](./adr/0017-record-images.md)), shown on cards, as list thumbnails and, for NPCs and deities, beside their names in a place popover (map pins were revised away by the DM). Open: a sweep for orphaned uploads (ADR-0017). See the spec's §11.
- **PWA / offline** for table use without reliable wifi.
- **A design system.** _(Noted 2026-09-17, unspecified.)_ None exists today, and [`README.md`](../README.md) says so. What there is instead: Tailwind utilities written directly in markup (CLAUDE.md rule 8), about 120 distinct raw colour classes across six grey-ish families (`gray` alone is used ~460 times, next to `zinc`, `slate` and `stone`), `dark:` variants in 24 files with no theme behind them, and a handful of shared components (`BaseButton`, `Modal`, `PageTitle`, `ListPage`, `Spinner`, `Toaster`) that nothing documents as a set. A spec would decide: semantic tokens as Tailwind v4 `@theme` variables in `app/ui/global.css` (surface, text, border, accent, danger) replacing raw palette names; whether dark mode is a goal or the stray `dark:` classes go; which components are the sanctioned primitives, and where their variants are written down; and how to stop drift (a lint rule against raw palette classes, as TD-72 did for inline `style`). **Finding while noting this:** `tailwind.config.ts` is not loaded. Tailwind v4 only reads a JS config through an `@config` directive, and `global.css` has none, so its custom `blue` shades, the `shimmer` keyframes that `skeletons.tsx` animates with, and the `@tailwindcss/forms` plugin are all absent from the built CSS (checked 2026-09-17: no `@keyframes shimmer`, and `--color-blue-500` is v4's default). That is [TD-112](./TECH_DEBT.md), whichever way this entry goes, and it is not blocked on the spec. The 2026-09-17 design critique's other findings are TD-113–TD-121; TD-118 (public vs admin list styling) is one this spec would still settle. **TD-115 took the "stray classes go" branch (2026-09-17):** the app is light-only now, so this spec starts dark mode from zero rather than finishing a half-built one — see TD-115's Resolution for exactly what was removed.
