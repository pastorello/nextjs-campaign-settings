...
- **TD-21 is not fully closed.** Every item in the original fix list is done except **step 3, `PageMeta.label`/`.placeholder`** — the field labels and placeholders shown in every form and list column, not the option-list labels ADR-0007's `SelectOption`/`ResolvedOption` split covers. Those ~45 field declarations across `app/lib/config/**` are still bare English string literals (`label: "Level"`), unresolved by anything steps 1–7 touched. This is real, sizeable remaining work — not a rounding error — and per the 2026-07-30 progress note it was deliberately deferred rather than folded into ADR-0007's scope. Do not close this item until that lands.
