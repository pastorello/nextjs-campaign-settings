...
**Open items:** TD-37 – TD-43, opened 2026-08-01. Every correctness/security item from the original audit is done; what is left is coverage — Phase 2's exit criterion (`docs/ROADMAP.md`) has always required 70% and the suite sits at 27.6% lines. TD-37–TD-43 slice that gap by area, ordered by how delicate the area is (auth/DB bootstrap first, presentation-only code last), matching the risk tiers already defined in `docs/TESTING.md` §2. No feature work — every item below adds tests against existing behaviour, nothing more.
...
| TD-37 | `authenticate()` and `app/lib/connections/**` are 0% covered — the login and DB-bootstrap path | 🟠 High               | S      | 2     |
...
### TD-37 🟠 `authenticate()` and `app/lib/connections/**` are 0% covered
