...
- **Remaining for TD-21:** step 7 (CI check that `messages/it.json` and `messages/en.json` have identical key sets — the two are hand-verified equal as of this session, via a one-off script, not enforced). `PageMeta.label`/`.placeholder` itself (the field labels shown in forms, not the option lists ADR-0007 covers) is also still English-only string literals — noted in the ADR-0007 progress block above as never migrated, and still true.
