...
- **Opening a Radix overlay twice in one file** — `vitest.setup.ts` resets focus in a `beforeEach`, and that hook is load-bearing. jsdom 30.1.0 keeps pointing at a focused element after Testing Library unmounts it, so the next `focus()` fires a stray `blur` at `window`; Radix's menu root closes on exactly that, and the second and later opens in a file died in the same tick. The hook is a `beforeEach` on purpose — Vitest runs `afterEach` hooks in reverse registration order, so anything declared in `vitest.setup.ts` runs after Testing Library's cleanup, too late to blur an attached node. `MapUser.test.tsx` is the suite that catches it.
...
| `a11y.spec.ts`            | `@axe-core/playwright` over each main page (supports TD-15).                                                                         | ✅ **zero**-violation gate, eleven pages plus a data-dependent campaign/adventure test (SPEC-013 T10), and a keyboard focus test |
...
- **`a11y.spec.ts`** — no longer differs. It shipped under TD-24 as a known-violations allowlist because a zero gate would have been red on arrival; **TD-15 then closed the violations and made it a zero gate** (`expect(summary).toEqual([])`). What the allowlist contained: `color-contrast` on every primary button — white on `violet-500` measured 4.4:1 against the 4.5:1 that 14px text needs, missing by a tenth — plus `link-name` on the icon-only sidebar and pagination links, and `button-name` on the sort controls. One entry, `aria-toggle-field-name`, had already been fixed before the pass began, which is exactly the failure mode an allowlist has. SPEC-013 T10 added the campaign and adventure pages as a separate test in the same file rather than two more `PAGES` entries: what those pages render depends on data (the campaign page is a creation form until the one campaign exists; the adventure page needs a real id in its URL), so the test builds its fixtures first, idempotently, the same way `world.setup.ts` does for the root place — the campaign has no delete action (T6, deliberate), so "E2E Campaign" persists in the e2e database after the first run, like the root place does. Each page is scanned with its add-forms open, so the bespoke ADR-0011 forms are inside the scan too.

**Three selector traps.** The `role="dialog"` element has no bounding box (its children are `fixed`), so Playwright reports it hidden — assert on something inside it. And each dialog renders its title twice, so a heading query inside one is a strict-mode violation. And on the map, `MapPOIPanel` covers the leftmost 384px of the map container whenever it is open — which it is for the whole of any flow that has just saved a place, since a save returns the panel to list view. A marker created there is painted over and cannot be pressed, so a spec driving a real mouse gesture (`page.mouse.down()`, not `dispatchEvent`) must place its target clear of it; check with `document.elementFromPoint` at the marker's centre when a gesture mysteriously does nothing. That is TD-101 in one line: it cost a quarantined spec and five days of suspecting `L.Draggable`.
