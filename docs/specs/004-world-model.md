...
- [ ] **T4** — NPC/deity pins for existing records; derive and display location from the tree. **Pins done 2026-08-07**, display deferred: `app/seed/migrateEntityPinsT4.ts` (idempotent, run by hand) created a pin for every NPC (119) and deity (5), parented on the place their `location` column already names (deities pin at `location`, not `residence` — per §6, the plane is reachable by walking up from there, so pinning at `residence` directly would skip a level). All null `lat`/`lng` — nobody has been placed on a map yet, only assigned a parent. One pre-existing DB row, `deities` id 1 "Divinità di prova", was the DM's own test data (confirmed 2026-08-07) and was deleted before this ran, not pinned. **Still open:** the "derive and display location from the tree" half — `npcMeta.ts`/`deityMeta.ts` still show the raw `location` dropdown backed by `locationList`, nothing yet reads a record's place by walking up its pin. Deferred deliberately (2026-08-07) as its own UI decision (where to show it — list column, detail page, both) rather than bundled into the data migration.
