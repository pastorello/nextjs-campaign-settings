...
- **Status:** Accepted — MVP through T4 and T6 shipped 2026-08-06/07 ([ADR-0009](../adr/0009-world-tree-as-one-polymorphic-table.md)); T5a (everything the drop depends on) shipped 2026-08-07 with its verification gate green on real data. **Only T5b — the drop itself — remains;** see §10
- **Date:** 2026-08-06
- **Phase:** 3
...
- [ ] **T5b** — The drop itself: `npc.luogo`, `deities.luogo`, `deities.residenza`, plus `locationList`, `celestialPlanes`, the now-orphaned `Location`/`PlaneOfExistence` enums, their 44 + 7 catalogue keys × 2 locales, the fields' entries in `npcMeta`/`deityMeta`/`pagesConfig`/`formFields`/`listConfig`/`queryFields`, the `location`/`residence` values in `app/seed/initial-data/`, `importLibrary.ts`'s `luogo` mapping, and `e2e/deities-list.spec.ts`'s Residenza regression test. _(The point of no easy return — write the down migration first, and re-run the T5a verifier immediately before.)_
