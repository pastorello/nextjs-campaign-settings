...
- **Status:** Shipped 2026-08-08. MVP through T6 landed 2026-08-06/07 ([ADR-0009](../adr/0009-world-tree-as-one-polymorphic-table.md)); T5b (the drop itself) closed the spec — `npc.luogo`, `deities.luogo` and `deities.residenza` are gone, along with the `Location`/`PlaneOfExistence` enums, `locationList`/`celestialPlanes`, and every dependent field in the metadata layer, forms and lists. See §10.
- **Date:** 2026-08-06
- **Phase:** 3
...
- [x] **T3** — Migrated the four existing maps and the 33 legacy places into the tree, DM-reviewed against the parentage evidence in §6. `app/seed/migrateWorldTreeT3.ts` (idempotent, run by hand — see its own header) seeded: the root (`piani-esistenza.jpg`) → 7 planes → `Terra` (the material world plane, carrying `mondo-materiale.jpg`) → `Regno di Kang` → `Skreebars`; then the 32 remaining named locations (Skreebars is the 33rd, already created by the map chain) as unplaced leaves, `kind` per `Location.ts`'s own section comments (Città→city, Dungeon→dungeon, else→region). Only 5 of 33 have direct parentage evidence (the deity `residence`/`location` pairs); the other 27 default to `Terra`, confirmed with the DM 2026-08-06 as a freely-reparentable starting point, not a claim of accuracy. Pre-migration audit found the dev DB's only `poi` rows (5) were e2e test debris (no real DM-created root existed yet) — deleted before seeding. Does not touch `npc`/`deities` rows (T4) or drop any column (T5).
- [x] **T4** — NPC/deity pins for existing records; derive and display location from the tree. **Pins done 2026-08-07**: `app/seed/migrateEntityPinsT4.ts` (idempotent, run by hand) created a pin for every NPC (119) and deity (5), parented on the place their `location` column already names (deities pin at `location`, not `residence` — per §6, the plane is reachable by walking up from there, so pinning at `residence` directly would skip a level). All null `lat`/`lng` — nobody has been placed on a map yet, only assigned a parent. One pre-existing DB row, `deities` id 1 "Divinità di prova", was the DM's own test data (confirmed 2026-08-07) and was deleted before this ran, not pinned. **Display done 2026-08-07:** the DM's call was to show it in the admin lists, not the detail page, and computed in one batch rather than cached on the entity — a cached column would reintroduce exactly the second-source-of-truth problem §6 removed `npc.location`/`deities.residence` to fix. `deriveEntityLocations` (`app/modules/maps/lib/utils/`) is a pure function: given the whole `poi` table (one query, `fetchDerivedLocations`) it resolves every linked NPC/deity to its pin's parent title in memory — no per-row round trip. Wired into `EntityList` as a `derivedLocation` column, present in `pageMetaFields` (so `listConfig`/`renderFieldValue` can render it) but deliberately absent from every `pagesConfig` entry, so `buildEntitySchema` never treats it as writable. `npcMeta.ts`'s raw `location` dropdown is untouched — both are visible until T5 decides the old one's fate.
- [x] **T5a** — Everything the drop depends on, while every column still exists. Purely additive and fully reversible; nothing here changes the schema. Scoping T5 turned up four things its one-line description assumed were already true and that were not:
  - **The derivation walked one level, and the deity card needs two.** `DeityCard` shows "Paradiso, Cieli" — the place _and_ the plane containing it — and the plane is not positionally the second ancestor (an NPC in Skreebars has Regno di Kang there). `deriveEntityAncestry` replaces T4's `deriveEntityLocations`, returning the whole chain nearest-first, with `findAncestorOfKind` for "the nearest ancestor that is a plane". Cycle-guarded: the mutation boundary rejects reparenting under a descendant, but Postgres permits a cycle on a self-referencing FK, and a corrupt row must not hang a render.
  - **Only the admin list was on derived data.** T4 wired `EntityList`; the public cards still resolved the raw columns. `EntityLibrary` now resolves placements server-side (`toDerivedPlacements` — plain strings, since the libraries are client components) and `NpcCard`/`DeityCard` render those instead.
  - **The verification gate.** `app/seed/verifyDerivedLocationsT5.ts` (read-only, run by hand) asserts, per record, that it has a pin, that the pin's parent is the place `location` names, and — deities only — that the nearest plane above it is the one `residence` names. **Run 2026-08-07: 119/119 NPCs and 5/5 deities reproduce their columns from the tree.** That is what makes T5b provably lossless rather than a judgement call about whether the migrated tree "looks right", and it must be re-run immediately before T5b.
  - **`createNpc` writes `location` and creates no pin**, so any record created through the form since T4 would drift. The verifier is what catches it; it reports such a record as "no pin — created after T4's migration, never placed".
- [x] **T5b** — The drop itself, done 2026-08-08: `npc.luogo`, `deities.luogo`, `deities.residenza`, plus `locationList`, `celestialPlanes`, the now-orphaned `Location`/`PlaneOfExistence` enums, their 44 + 7 catalogue keys × 2 locales, the fields' entries in `npcMeta`/`deityMeta`/`pagesConfig`/`formFields`/`listConfig`/`queryFields`, the `location`/`residence` values in `app/seed/initial-data/`, `importLibrary.ts`'s `luogo` mapping, and `e2e/deities-list.spec.ts`'s Residenza regression test. The T5a verifier was re-run immediately before (119/119 NPCs, 5/5 deities, still lossless) and the migration (`prisma/migrations/20260808160000_drop_legacy_npc_deity_location_columns`) applied on that evidence. `app/seed/migrateEntityPinsT4.ts` and `app/seed/verifyDerivedLocationsT5.ts` were both deleted in the same change — one-time, hand-run scripts whose entire job was reading the columns just dropped; their results (T4's 119+5 pins, T5a's 119/119+5/5 verification) are recorded in this file's history, not in runnable code.

  **Two consequences agreed with the DM 2026-08-07, both deliberate:**

  - **The NPC location filter goes and is not rebuilt.** `queryFields` makes `npc.location` filterable and `NpcLibrary` renders a live `SelectButtonery` for it; neither survives a dropped column. Rebuilding it on the tree means resolving a chosen place to its descendants' pins and filtering `npc.id IN (…)` — workable — but the control's options would have to come from the database, which is the async-options problem §7 leaves explicitly unsolved, making the replacement larger than T5 itself. **Rejected in favour of accepting the loss:** the map already answers the same question better (click Skreebars, see who is there — `fetchPlaceChildren`). Do not re-propose rebuilding the filter as a prerequisite for T5b; it is a separate feature if it is ever wanted.
  - **The NPC/deity form loses its location dropdown, and the map becomes the only way to place a record.** That is §5 point 6 working as designed ("there is no location dropdown to keep in sync"), but it is a workflow change: create the record, then pin it from the map. Until pinned, its location renders blank — a valid state per §5's edge-case table, not a bug.

- [x] **T6** — Docs: [ADR-0009](../adr/0009-world-tree-as-one-polymorphic-table.md) for the tree model, `ARCHITECTURE.md`'s maps section updated, `ROADMAP.md`'s "Real relations"/"Locations as first-class entities" bullets rewritten to reflect what actually shipped instead of the superseded SPEC-003 plan

## 11. Outcome

Shipped in full 2026-08-08. The `poi` tree is now the single source of truth for where an NPC or deity is: the flat `location`/`residence` columns and their closed enum vocabularies are gone, and placement is entirely a function of a record's pin in the tree. Two deliberate losses came with it (§10, T5b): the NPC admin list's location filter was not rebuilt (the map answers the same question), and the NPC/deity forms no longer offer a location dropdown — placement happens by pinning on the map after a record is created. SPEC-006 (table-backed options) and SPEC-007 (placement backlog) were both blocked on this closing and can now proceed.
