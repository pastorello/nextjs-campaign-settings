...
**The count is a tree-wide read, not a per-parent one.** `useUnplacedChildren` answers "which children of _this_ place lack coordinates" and stays as it is. _(It did, until [SPEC-017](./017-one-unplaced-pool.md) T8 replaced it with `useUnplacedPlaces`: the per-parent scoping was what made a place under the wrong parent unreachable from every other map, since the list came from the very tree edge the DM was trying to change.)_ The new count is `prisma.zone.count({ where: { lat: null, parentId: { not: null } } })` — every place except the root. One query, on a table of 42 rows.
