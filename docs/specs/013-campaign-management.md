...
- [x] **T10** — i18n both catalogues, a11y pass, docs update. **Done 2026-08-19.** The i18n half was verification rather than work: T7–T9 had already landed every key in both catalogues (CI's key-set check enforced it as they shipped), and a scan of `app/ui/campaigns/**` and the two pages found no hardcoded copy. The a11y half extended `e2e/a11y.spec.ts` with a campaign+adventure test — a separate test, not two `PAGES` entries, because both pages are data-dependent; it builds its fixtures idempotently the way `world.setup.ts` does (the campaign has no delete action, so "E2E Campaign" persists in the e2e database after the first run) and scans each page with its bespoke add-forms open, so `AdventureForm`/`SceneForm`/`SceneCreatureForm`/`LootForm` are inside the scan. axe came back at zero on both pages; what the test _did_ catch was in its own fixture setup: a minimal title-only campaign create failed validation because `campaignMeta.synopsis` was a bare `.optional()` while `CampaignForm` submits an unset synopsis as an explicit `null` — the same gap T7 found on `adventureMeta.synopsis`/`timeline` and T8 on `sceneMeta.description`/`sceneCreatureMeta.note`, fixed with the same `nullableToOptional` preprocess and regression-tested in `campaignMeta.test.ts`. Not scanned: the edit variants of the forms (same components and inputs, different initial values) and creature/loot rows (same row controls the scene row's scan covers). Docs: `PROJECT_STATE.md`, `ROADMAP.md`, `TESTING.md`, this file's §11; `ARCHITECTURE.md` needed nothing — T7–T9 kept its ADR-0011 section current as they landed. _(test: catalogue key-set check in CI, axe at zero)_
...
_Closed 2026-08-19, all ten tasks done — T1 on 2026-08-18, the rest across the
following day. Each task's entry in §10 carries its own detail; this is the
summary._

- **Shipped:** the whole of §5 — the campaign page with its adventure ladder,
  the adventure page with scenes/creatures/loot edited inline, the four-budget
  panel with hero-point count, at-the-table check-off that survives a reload,
  the treasure catalogue as the seventh domain, and `magicitems.consumable`
  with its backfill. Bilingual throughout, axe at zero on both new pages,
  keyboard operation tested.
- **Deviations from spec and why:** three amendments are already marked in
  place (§6's Prisma block, §9's mutations directory, §3's import non-goal).
  Beyond those: `campaign`/`adventure` landed bespoke rather than as
  registered domains — [ADR-0011](../adr/0011-inline-collections-outside-the-metadata-layer.md)'s
  2026-08-19 amendment, the ladder reading of §5, confirmed by the DM.
  `campaign` got no delete action (§5 names no flow for removing the campaign
  itself, and its `adventure` FK is `Restrict`). Reordering is up/down
  buttons, not drag-and-drop, per §9's laptop-only interaction note.
- **Follow-up debt created:** none registered. Two residuals worth naming:
  the CSV/spreadsheet import stayed deferred (§3, a `ROADMAP.md` candidate,
  not debt), and §9's "50-scene adventure" volume risk remains untested —
  every screen has only seen hand-made fixtures until the DM authors real
  content.
- **A pattern worth recording:** all four bespoke meta files needed the same
  `nullableToOptional` fix, found one file at a time (T7 in the browser, T8
  in the browser, T10 by the a11y e2e's fixture setup). The domain interfaces
  type unset strings `| null`; a bare `.optional()` validator rejects that.
  Any future bespoke meta file should start from that shape.
