...
- [x] **T3** — The date systems panel at `/world/calendar`: universal count names, create/edit/delete systems, default, moon reference. _(test: actions — auth, validation, universal/default undeletable)_
      — **Done 2026-09-18.** A shared page (ADR-0013; not a catalogue page, so no `pagesConfig` entry), linked from `/world`; no sidebar entry, since no world page has one. Validators and label keys in `app/lib/config/calendar/` (`dateSystemMeta`, `calendarSettingsMeta`; `dateSystemSchemas` composes them with their types kept). Actions in `app/lib/data/calendar/`: `updateUniversalDateSystem` (names and "after" labels only — an anchor in the payload is stripped), `createDateSystem`, `updateDateSystem` (refuses the universal row), `deleteDateSystemById` (refuses the universal count and the default, guarded in the delete's own `where`), `setDefaultDateSystem` (clear-then-set in one interactive transaction, rolled back if the target is gone, so there is always exactly one default), `setMoonReferenceDay` (`null` clears). Refusals are `FieldErrorKey`s on `id`; 12/7 names have their own keys (`monthNamesCount`, `weekdayNamesCount`). The editor (`DateSystemsPanel`, `DateSystemForm`, `MoonReferenceForm`) warns as soon as an anchor year differs from the saved one. The moon reference is shown in the viewer's display system beside the toggle, so re-defaulting re-labels it on the page. Tests: every action, the meta, the three components, the page; `e2e/calendar-date-systems.spec.ts` restores the universal count as default in a `finally`.
- [x] **T4** — The date input and date display components, and the per-viewer system toggle. _(test: input round-trip; display in two systems)_
      — **Done 2026-09-18.** Reads: `app/lib/data/calendar/` — `fetchDateSystems` (universal first, then by name), `fetchDefaultDateSystem`, `fetchCalendarSettings`, `readDisplayDateSystemId`. Pure: `formatWorldDate` (→ weekday, day, month, "330 a.C.", "14:00"; the join is `calendar.date.*` copy), `parseWorldDateFields` / `worldDateFieldsFromDay` (typed fields ↔ universal day), `resolveDisplayDateSystem`. Components in `app/ui/calendar/`: `WorldDate` (not a client component), `WorldDateInput`, `DateSystemToggle`. **The date control is bespoke, not a `PageMeta` control type** — ADR-0011's test: its value spans two columns (day + hour), it needs the date systems as data no `ControlProps` control receives, and neither a list column nor a header filter could use a "world date" control type (§5.6's filters are by links, not dates), so a variant would be a not-applicable branch in three of four consumers. T5/T6's `calendarEventMeta` still declares the date fields' validators and label keys — `universalDayValidator` (whole, ≥ 0, ≤ 2³¹ − 1 for the `integer` column) and `worldHourValidator` — and the form hands them to the control. Field errors use two new keys, `beforeDawnOfTime` (on the year) and `dayNotInMonth` (on the day), the same keys the validator refuses with; a too-large year is `tooBig`. **The toggle is a cookie** (`worldDateSystem`, a system id, one year, `SameSite=Lax`), written client-side then `router.refresh()`: dates render in Server Components, which read the cookie on the request and render in the chosen system the first time — `localStorage` would force client-only rendering and a flash of the default. A stale id falls back to the default. Hours read "09:00" as well as "14:00" (zero-padded, so a column lines up). Tests: `worldDateInput.test.ts`, `formatWorldDate.test.ts`, `displayDateSystem.test.ts`, the data reads, `universalDayValidator.test.ts`, and the three components.
- [x] **T5** — World history at `/world/history`: CRUD with links, chronological list with filters. _(test: actions; list order; filters)_
      — **Done 2026-09-18.** **How much of the metadata layer applies:** the fields' _declarations_, not the generic list. `calendarEventMeta` (title, description, start/end day and hour, `repeatsYearly` — shared with T6) and `worldHistoryLinkMeta` (`zoneIds`/`npcIds`/`deityIds`/`factionIds`, multiselects over their tables; `deities` joined `OptionTableName`) give every field its validator and label key, but world history is **not** a `PageType`/`pagesConfig` entry: its list is grouped by year and month in the displayed system, its filters are by link rather than by column, and its dates need T4's bespoke control — so neither `ListPage`'s table, `getQuery`'s filters nor `EntityForm` fit, and registering it would be a not-applicable branch in each. It is a shared page (ADR-0013) the way `/world` and `/geography` are, by not being a catalogue; `switchSystemPath.test.ts` pins it. **Data** (`app/lib/data/calendar/`): `createWorldHistoryEvent`, `updateWorldHistoryEvent` (replaces every field; links with `set`), `deleteWorldHistoryEventById` — auth, then `worldHistoryEventSchema` (`calendarEventShape` + links, refined by `checkCalendarEventDates`: end not before start, including the hour on the same day; an end hour needs an end day; a yearly event's end at most 364 days after its start), then `findMissingEventLinks` (`zoneNotFound`/`npcNotFound`/`deityNotFound`/`factionNotFound` on the list field). A campaign event is not found to these actions. T6 reuses `calendarEventShape` + `checkCalendarEventDates` with its own owner fields. New keys `endBeforeStart`, `repeatSpansOverAYear`; `revalidateDashboard("world/history")`. **Read:** `fetchWorldHistory` — campaign-less events by start day (a day's untimed events first), filtered by `?place=&npc=&deity=&faction=` (all set must match; direct links only, a place's sub-places are not included), **paged by year**: 10 years that hold events per page, never splitting a year; a yearly event is listed once, where it starts. **Page:** `/world/history`, linked from `/world` — `DateSystemToggle`, `WorldHistoryFilters`, `WorldHistoryList` (year and month headings via `groupEventsByYearAndMonth`, a "yearly" marker, places linking to their map and NPCs/deities/factions to their list filtered by name, as the search page does), `WorldHistoryEventForm` (the end behind a checkbox, since `WorldDateInput` reports `null` for both blank and half-typed). Tests: the schema, the three actions, the read, the URL parser, the grouping, the metas, the three components, `e2e/world-history-crud.spec.ts`.
- [x] **T6** — Campaign calendar at `/campaign/calendar`: CRUD with adventure/scene and yearly repeat, current day, past/upcoming, adventure filter, the next three upcoming events on the campaign page. _(test: scene/adventure consistency; past/upcoming)_
      — **Done 2026-09-18.** **Actions** (`app/lib/data/calendar/`): `createCampaignEvent(campaignId, input)`, `updateCampaignEvent` (never moves an event to another campaign), `deleteCampaignEventById` — auth, then `campaignEventSchema` (`calendarEventShape` + `campaignEventOwnerMeta`'s `adventureId`/`sceneId`, refined by `checkCalendarEventDates`; a non-empty `zoneIds`/`npcIds`/`deityIds`/`factionIds` is refused as `campaignEventNoLinks` rather than silently stripped), then `resolveCampaignEventOwner` (§5.4: `adventureNotInCampaign`, `sceneNotInAdventure`, `sceneNotInCampaign`; with only a scene named, the adventure is set from it). `findCampaignEvent` mirrors `findWorldHistoryEvent`, so neither page can touch the other's rows. `setCampaignCurrentDay` (validator in `campaignCalendarMeta`, not `campaignMeta` — the campaign form does not edit it). **Past/upcoming** is pure: `eventTiming` (past = ended before today, current = spans it, upcoming = after; a yearly event by its occurrence on or after today, carried whole across the year's end, so never past; no today → nothing marked) and `upcomingEvents` (the next _n_ strictly upcoming, soonest first). **Page:** `CurrentDayForm` (set, advance one day, clear), `CampaignCalendarFilters` (`?adventure=`; another campaign's id reads as "all"), `CampaignCalendarList` — past dimmed, current highlighted, each also marked in words; world history of the same whole years (`yearSpan` over the events and today, `fetchWorldHistoryBetween`) interleaved, dashed and read-only. **Generalised from T5 rather than copied:** `EventList` (grouping, add/edit/delete, confirm dialog), `EventSummary` (title, yearly marker, date range, description) and `CalendarEventFields` + `calendarEventDraft` (the shared form fields) now serve both lists; `WorldHistoryList`/`WorldHistoryEventForm` are thin wrappers over them. The campaign page links the calendar and shows `UpcomingEvents` (next three, a yearly one on its next date; nothing without a today). The adventure page shows `adventure.timeline` with a "move this into calendar events" note (T8 prep); the adventure form still edits it until T8. **Not done:** a yearly world history event first held before the range shown does not appear in the campaign list (its later occurrences are the grid's, T7). Tests: the four actions, the timing helpers (incl. a yearly event across a year boundary), `yearSpan`, the reads, the components, both pages; `e2e/campaign-calendar.spec.ts` (creates the dnd5e campaign if the test DB has none — there is no campaign delete — and clears today at the end, so a today set before the run is not restored).
- [x] **T7** — The month grid, shared by T5 and T6. _(test: weekday alignment across a year boundary; multi-day events; e2e for one month)_
