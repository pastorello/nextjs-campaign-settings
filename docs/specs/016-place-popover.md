...
> **Renamed and merged since (SPEC-023, 2026-09-24).** T5's "Sposta nei luoghi non posizionati" and T6's "Rimuovi definitivamente" are one "Rimuovi" entry opening a dialog with both outcomes, for a landmark as for a zone; `DeletePlaceButton` is `RemovePlaceDialog`, and the `geography.deletePlace` messages are `geography.removePlace`. The task text above is what T6 and T7 built, kept as-is.
