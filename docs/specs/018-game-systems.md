...
- [x] **T2** — The system dimension, with 5e as the only system: `GameSystem`, the `[system]` URL segment with redirects from old URLs, the page `system` field, and the switch. No visible change beyond the URL. _(test: page-classification unit test; redirect tests; full e2e green)_
  ...
  - [x] _Part B_ — rewrite every file in the scan's allowlist onto `dashboardPath` until it is empty, then the unit tests and e2e `goto`s that still spell `/dashboard/…`. Full e2e once at the end. _(Verified 2026-09-17: `i18n/dashboardPaths.test.ts` has no allowlist; the only system-less paths left are deliberate — `proxy.test.ts`'s matcher and redirect cases, and `e2e/not-found.spec.ts`'s two redirect/404 checks.)_
  - [x] _Part C_ — the switch (ADR-0013 rule 6) and search filtered by the page's `system` (rule 10). Asked the DM whether the switch is hidden or shown disabled while `dnd5e` is the only system; **answer (2026-09-18): shown disabled, not hidden.**
    - [x] _Search_ (rule 10) — done 2026-09-17. `searchAllDomains(term, system)` skips, without querying, any domain whose `pagesConfig` page belongs to another system; NPCs, deities, factions and places are always searched. The domain list stays hand-written, so campaign content remains out. Nothing changes while `dnd5e` is the only system.
    - [x] _Switch_ (rule 6) — done 2026-09-18. `SystemSwitcher` sits in the sidebar above the locale switcher: a `<select>` over `GAME_SYSTEMS`, labelled from `common.nav.gameSystem`, valued from `useGameSystem`. With one system it is `disabled` and described by a screen-reader hint (`common.nav.gameSystemSingle`). The target comes from `switchSystemPath` (`i18n/`): a shared page keeps its path and query; a page whose `pagesConfig` entry belongs to another system (`/spells`, `/admin/spells/new`, `/spells/12/edit`) goes to the target's overview, as rule 6 says.
- [x] **T3** — `campaign.system`, backfilled to `dnd5e`. Creating a campaign asks for it, the campaign list is filtered by the URL's system, and a campaign opens under its own system. _(test: backfill; list filter; redirect)_
  - _Done 2026-09-17._ Additive migration `*_spec018_campaign_system` (`NOT NULL DEFAULT 'dnd5e'`, which is the backfill; English column name, like the rest of the table). `campaignMeta.system` declares the Select and its `z.enum(GAME_SYSTEMS)` validator; `CampaignForm` shows it on create only, preselected from the route, and `updateCampaign` strips it, because a campaign does not switch systems. `fetchCampaign(system)` filters, so another system's URL shows the empty state. "Opens under its own system" is enforced on the adventure page, the only campaign URL that names a record: it redirects (keeping the locale) to the owning campaign's system. A standalone adventure (`campaignId` null) has no system and opens under any.
- [x] **T4** — Slice spec: Daggerheart domains, domain cards, classes, subclasses — [SPEC-021](./021-daggerheart-domains-and-classes.md), agreed and shipped 2026-09-19, after [SPEC-019](./019-formatted-text.md) and [SPEC-020](./020-record-images.md). The first second-system catalogue: `daggerheart` is live in `GAME_SYSTEMS`, and search filtering by system (T2 part C) now has catalogues on both sides.
