...
- [ ] **T1** — ADR-0018 (features per owner). `daggerheart` in `GAME_SYSTEMS`; the sidebar section; schema and additive migration. _(test: page classification; migration)_
- [ ] **T2** — Domains: meta, actions, list/form/detail with colour and emblem. _(test: actions; delete refused when used)_
- [ ] **T3** — Domain cards: meta, actions, list with filters, the card view and the rows/cards switch. _(test: filters; card view render)_
- [ ] **T4** — Classes with inline ordered features and the Hope feature. _(test: distinct domains; ≥1 feature; reorder)_
- [ ] **T5** — Subclasses with tiered inline features. _(test: tiers; spellcast trait)_
